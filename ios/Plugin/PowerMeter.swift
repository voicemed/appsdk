...
//  App
//
//  Created by Francesco Lo Truglio on 08/09/22.
//
...
class PowerMeter: AudioLevelProvider {
    ...
    private static let skMinLevel: Float = 0.000_000_01 // -160 dB
    ...
    public static func getPowers(data: UnsafePointer<Float>, strideFrames: Int, length: vDSP_Length) -> PowerLevels {
        var max: Float = 0.0
        vDSP_maxv(data, strideFrames, &max, length)
        if max < skMinLevel {
            max = skMinLevel
